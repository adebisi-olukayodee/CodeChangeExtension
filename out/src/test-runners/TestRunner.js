...
class TestRunner {
        ...
        return new Promise((resolve, reject) => {
            ...
            process.on('close', (code) => {
                ...
                if (framework === 'vitest' && (testFile.endsWith('.ts') || testFile.endsWith('.tsx'))) {
                    ...
                    hasTypeErrors = typeErrorPatterns.some(pattern => pattern.test(stdout) || pattern.test(stderr));
                    if (hasTypeErrors) {
    ...
    determineTestStatus(exitCode, stdout, stderr, hasTypeErrors = false) {
