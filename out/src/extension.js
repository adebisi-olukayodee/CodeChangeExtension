 ...
 */
function extractBreakingIssues(result) {
...
function activate(context) {
    ...
    try {
        ...
        const impactAnalyzer = new ProfessionalImpactAnalyzer_1.ProfessionalImpactAnalyzer(configManager);
        const testRunner = new TestRunner_1.TestRunner();
        const gitAnalyzer = new GitAnalyzer_1.GitAnalyzer();
        // Initialize UI
        const viewProvider = new SimpleImpactViewProvider_1.SimpleImpactViewProvider(impactAnalyzer, testRunner);
        vscode.window.registerTreeDataProvider('impactAnalyzerView', viewProvider);
        // Initialize Welcome Panel (Month 1 Quick Win #1)
        const welcomeViewProvider = new WelcomeViewProvider_1.WelcomeViewProvider(context);
        context.subscriptions.push(vscode.window.registerWebviewViewProvider('impactAnalyzer.welcome', welcomeViewProvider));
        // Initialize Inline Decorations Manager (Month 1 Quick Win #3)
        const inlineDecorationsManager = new InlineDecorationsManager_1.InlineDecorationsManager(context);
                ...
                if (supportedExtensions.includes(fileExt)) {
                    ...
                    try {
                        ...
                        if (result.hasActualChanges !== false) {
                            ...
                            const quickSummary = ImpactSummaryFormatter_1.ImpactSummaryFormatter.formatQuickSummary(result);
                            const detailedSummary = ImpactSummaryFormatter_1.ImpactSummaryFormatter.formatDetailedSummary(result);
                            ...
                            if (breakingIssues.length > 0) {
                                vscode.window.showWarningMessage(quickSummary, 'View Details').then(selection => {
        ...
        let autoRefreshDisposable;
        const pendingAutoRefreshTimers = new Map();
        ...
        const runAutoRefresh = async (filePath, document) => {
            try {
        ...
        const registerAutoRefreshListener = () => {
            ...
            autoRefreshDisposable = vscode.workspace.onDidSaveTextDocument((document) => {
                ...
                if (pendingAutoRefreshTimers.has(filePath)) {
                    clearTimeout(pendingAutoRefreshTimers.get(filePath));
                }
        ...
        const commands = [
            ...
            vscode.commands.registerCommand('impactAnalyzer.analyzeCurrentFile', async () => {
                ...
                try {
                    ...
                    const confidenceExplanation = result.confidenceResult
                        ? ConfidenceMetricsExplainer_1.ConfidenceMetricsExplainer.generateExplanation(result.confidenceResult)
                        : '';
                    outputChannel.clear();
                    outputChannel.appendLine(detailedSummary);
                    ...
                    if (breakingIssues.length > 0) {
                        ...
                        vscode.window.showInformationMessage(`${conf.statusIcon} Confidence: ${conf.total}/100 (${conf.status}) - ` +
            ...
            vscode.commands.registerCommand('impactAnalyzer.runAffectedTests', async (testItems) => {
                try {
                    // Handle both array of strings and array of objects with file property
                    let tests = [];
                    if (Array.isArray(testItems)) {
                        tests = testItems.map(item => typeof item === 'string' ? item : (item?.file || item?.testFile || '')).filter(Boolean);
                    }
                    if (tests.length === 0) {
                    ...
                    if (failed > 0) {
                        vscode.window.showErrorMessage(`Tests completed: ${failed} failed, ${passed} passed, ${skipped} skipped`, 'View Results').then(selection => {
                        ...
                        vscode.window.showWarningMessage(`Tests completed: ${passed} passed, ${skipped} skipped`);
                    }
                    else {
                        vscode.window.showInformationMessage(`✅ All tests passed! (${passed}/${results.length})`);
                    }
                }
                catch (error) {
