...
const debug_logger_js_1 = require("../../core/debug-logger.js");
class TypeScriptAnalyzer {
    ...
    async buildSnapshot(filePath, content) {
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========================================`);
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Building snapshot for: ${filePath}`);
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Timestamp: ${new Date().toISOString()}`);
        const preview = content.replace(/\s+/g, ' ').slice(0, 160).trim();
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Content length: ${content.length} chars, preview="${preview}${content.length > 160 ? '...' : ''}"`);
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========================================`);
        try {
    ...
    async diffSnapshots(beforeSnapshot, afterSnapshot) {
        ...
        // Log interface call signatures for debugging
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========== INTERFACE COMPARISON ==========`);
        for (const [name, beforeIntf] of beforeInterfaces) {
            const callSig = beforeIntf.metadata?.callSignature;
            if (callSig) {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] BEFORE interface '${name}' call signature:`);
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    ${JSON.stringify(callSig, null, 2)}`);
            }
        }
        for (const [name, afterIntf] of afterInterfaces) {
            const callSig = afterIntf.metadata?.callSignature;
            if (callSig) {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] AFTER interface '${name}' call signature:`);
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    ${JSON.stringify(callSig, null, 2)}`);
            }
        }
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========== END INTERFACE COMPARISON ==========`);
    ...
    compareSymbols(beforeMap, afterMap, kind, changedSymbols, added, removed, modified, suppressedSymbols = new Set()) {
        ...
        for (const [name, afterSymbol] of afterMap) {
            ...
            if (beforeSymbol) {
                // Check if export status changed (exported -> not exported is breaking)
                if (beforeSymbol.isExported && !afterSymbol.isExported) {
                    // Symbol was exported before but is no longer exported
                    // This is a breaking change even if the symbol still exists
                    changedSymbols.push({
                        symbol: afterSymbol,
                        changeType: 'removed',
                        before: beforeSymbol,
                        ...
                        metadata: {
                            ...
                            message: `Symbol '${name}' is no longer exported (export keyword removed)`,
                            detail: 'exportRemoved'
                        }
                    });
                    // Continue to check for other changes (signature, etc.)
                }
    ...
    detectParameterBreakingChange(before, after) {
        ...
        const beforeHasRest = before.parameters.some(p => p.rest === true);
        const afterHasRest = after.parameters.some(p => p.rest === true);
        ...
        for (const [paramName, afterParam] of afterParams) {
            ...
            if (beforeParam) {
                ...
                const beforeIsRest = beforeParam.rest === true;
                const afterIsRest = afterParam.rest === true;
    ...
    detectPropertyBreakingChange(before, after) {
        ...
        // FIRST: Check if interface has a call signature (interface is callable like a function)
        // Example: interface Transformer { (data: any, headers?: AxiosRequestHeaders): any; }
        if (before.kind === 'interface' || after.kind === 'interface') {
            const beforeCallSig = before.metadata?.callSignature;
            const afterCallSig = after.metadata?.callSignature;
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========== INTERFACE CALL SIGNATURE COMPARISON ==========`);
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Interface: '${before.name}'`);
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Before call signature exists: ${!!beforeCallSig}`);
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] After call signature exists: ${!!afterCallSig}`);
            if (beforeCallSig) {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] BEFORE call signature:`);
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    Parameters: ${JSON.stringify(beforeCallSig.parameters.map(p => ({
                    name: p.name,
                    type: p.type,
                    optional: p.optional
                })))}`);
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    Return type: ${beforeCallSig.returnType}`);
            }
            if (afterCallSig) {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] AFTER call signature:`);
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    Parameters: ${JSON.stringify(afterCallSig.parameters.map(p => ({
                    name: p.name,
                    type: p.type,
                    optional: p.optional
                })))}`);
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    Return type: ${afterCallSig.returnType}`);
            }
            if (beforeCallSig || afterCallSig) {
                if (beforeCallSig && afterCallSig) {
                    // Both have call signatures - compare them
                    const beforeParams = new Map(beforeCallSig.parameters.map(p => [p.name, p]));
                    const afterParams = new Map(afterCallSig.parameters.map(p => [p.name, p]));
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Comparing call signatures parameter by parameter...`);
                    // Check for parameter optionality changes
                    for (const [paramName, afterParam] of afterParams) {
                        const beforeParam = beforeParams.get(paramName);
                        if (beforeParam) {
                            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   Parameter '${paramName}': before.optional=${beforeParam.optional}, after.optional=${afterParam.optional}`);
                            if (beforeParam.optional && !afterParam.optional) {
                                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   ✅ BREAKING CHANGE DETECTED: Parameter '${paramName}' changed from optional to required`);
                                return {
                                    changeType: 'signature-changed',
                                    isBreaking: true,
                                    ruleId: 'TSAPI-FN-001',
                                    message: `Interface call signature parameter '${paramName}' changed from optional to required`
                                };
                            }
                        }
                    }
                    ...
                    for (const [paramName, beforeParam] of beforeParams) {
                        if (!afterParams.has(paramName)) {
                            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   ✅ BREAKING CHANGE DETECTED: Parameter '${paramName}' was removed`);
                            return {
                                changeType: 'signature-changed',
                                isBreaking: true,
                                ruleId: 'TSAPI-FN-002',
                                message: `Interface call signature parameter '${paramName}' was removed`
                            };
                        }
                    }
                    // Check return type
                    if (beforeCallSig.returnType !== afterCallSig.returnType) {
                        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   ✅ BREAKING CHANGE DETECTED: Return type changed from '${beforeCallSig.returnType}' to '${afterCallSig.returnType}'`);
                        return {
                            changeType: 'type-changed',
                            isBreaking: true,
                            ruleId: 'TSAPI-FN-004',
                            message: `Interface call signature return type changed from '${beforeCallSig.returnType}' to '${afterCallSig.returnType}'`
                        };
                    }
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   No breaking changes detected in call signature`);
                }
                else if (beforeCallSig && !afterCallSig) {
                    // Call signature was removed
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   ✅ BREAKING CHANGE DETECTED: Call signature was removed`);
                    return {
                        changeType: 'signature-changed',
                        isBreaking: true,
                        ruleId: 'TSAPI-IF-003',
                        message: 'Interface call signature was removed (interface is no longer callable)'
                    };
                }
                else if (!beforeCallSig && afterCallSig) {
                    // Call signature was added (not breaking, but we note it)
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   Call signature was added (non-breaking)`);
                    // Continue to check properties
                }
            }
            else {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   No call signatures to compare`);
            }
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========== END CALL SIGNATURE COMPARISON ==========`);
        }
        ...
        // Special handling for function types: check if both are function types
        const beforeFunctionType = beforePropMap.get('__functionType');
        const afterFunctionType = afterPropMap.get('__functionType');
        if (beforeFunctionType || afterFunctionType) {
            if (beforeFunctionType && afterFunctionType) {
                // Both are function types - compare method signatures
                const beforeMethodSig = beforeFunctionType.methodSignature;
                const afterMethodSig = afterFunctionType.methodSignature;
                if (beforeMethodSig && afterMethodSig) {
                    ...
                    for (const [paramName, afterParam] of afterParams) {
                        const beforeParam = beforeParams.get(paramName);
                        if (beforeParam && beforeParam.optional && !afterParam.optional) {
                            return {
                                changeType: 'signature-changed',
                                isBreaking: true,
                                ruleId: 'TSAPI-FN-001',
                                message: `Function type parameter '${paramName}' changed from optional to required`
                            };
                        }
                    }
                    // Check for removed parameters
                    for (const [paramName, beforeParam] of beforeParams) {
                        if (!afterParams.has(paramName)) {
                            return {
                                ...
                                isBreaking: true,
                                ruleId: 'TSAPI-FN-002',
                                message: `Function type parameter '${paramName}' was removed`
                            };
                        }
                    }
                    // Check return type
                    if (beforeMethodSig.returnType !== afterMethodSig.returnType) {
                        return {
                            ...
                            isBreaking: true,
                            ruleId: 'TSAPI-FN-004',
                            message: `Function type return type changed from '${beforeMethodSig.returnType}' to '${afterMethodSig.returnType}'`
                        };
                    }
                }
            }
            else if (beforeFunctionType && !afterFunctionType) {
                // Function type was removed
                return {
                    changeType: 'signature-changed',
                    isBreaking: true,
                    ruleId: isTypeAlias ? 'TSAPI-TYPE-004' : 'TSAPI-IF-003',
                    message: 'Function type was removed'
                };
            }
            else if (!beforeFunctionType && afterFunctionType) {
                // Function type was added (not breaking, but we note it)
            }
        }
        // Check for property changes (optional -> required, type changed)
        for (const [propName, afterProp] of afterPropMap) {
            // Skip __functionType as it's handled above
            if (propName === '__functionType') {
                continue;
            }
    ...
    compareExports(beforeExports, afterExports) {
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========== compareExports START ==========`);
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Comparing exports: ${beforeExports.length} before, ${afterExports.length} after`);
        // Log all exports for debugging
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Before exports:`);
        for (const exp of beforeExports) {
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   - name='${exp.name}', kind='${exp.kind}', sourceModule='${exp.sourceModule}', sourceName='${exp.sourceName}'`);
        }
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] After exports:`);
        ...
        for (const exp of beforeExports) {
            ...
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Before export '${exp.name}': signature='${sig}'`);
        }
        for (const exp of afterExports) {
            ...
            const sig = this.getExportSignature(exp);
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] After export '${exp.name}': signature='${sig}'`);
        }
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Maps built: before has ${beforeMap.size} unique names, after has ${afterMap.size} unique names`);
        const added = [];
        const removed = [];
        const modified = [];
        // Find added exports
        ...
        for (const [name, beforeExportsList] of beforeMap) {
            if (!afterMap.has(name)) {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ✅ Export '${name}' was removed (not found in after map)`);
        ...
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Checking for modified exports...`);
        for (const [name, afterExportsList] of afterMap) {
            const beforeExportsList = beforeMap.get(name);
            if (beforeExportsList) {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Export '${name}' exists in both: before=${beforeExportsList.length} entries, after=${afterExportsList.length} entries`);
                ...
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   beforeReexport: ${beforeReexport ? `kind=${beforeReexport.kind}, sourceModule=${beforeReexport.sourceModule}, sourceName=${beforeReexport.sourceName}` : 'none'}`);
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   afterReexport: ${afterReexport ? `kind=${afterReexport.kind}, sourceModule=${afterReexport.sourceModule}, sourceName=${afterReexport.sourceName}` : 'none'}`);
                if (beforeReexport && afterReexport) {
                    ...
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Comparing re-export '${name}': before sig='${beforeSig}', after sig='${afterSig}'`);
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   Signatures equal: ${beforeSig === afterSig}`);
                    if (beforeSig !== afterSig) {
                        // Signature changed - sourceModule or sourceName changed
                        modified.push({ before: beforeReexport, after: afterReexport });
                        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ✅ Re-export '${name}' changed: ${beforeSig} → ${afterSig}`);
                    }
                    else {
                        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Re-export '${name}' unchanged (signatures match)`);
                    }
                }
                    ...
                    if (beforeExport && afterExport) {
                        ...
                        if (beforeSig !== afterSig) {
                            ...
                            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Export '${name}' changed: ${beforeSig} → ${afterSig}`);
                        }
                    }
                }
                else {
                    // One is re-export, one is not - this is a type change
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Export '${name}' type changed (re-export ↔ declaration)`);
                    modified.push(afterReexport || afterExportsList[0]);
                }
            }
        }
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Export comparison result: ${added.length} added, ${removed.length} removed, ${modified.length} modified`);
        // Log details of removed exports
        if (removed.length > 0) {
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ✅ Removed exports:`);
            for (const rem of removed) {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   - name='${rem.name}', kind='${rem.kind}', line=${rem.line}`);
            }
        }
        ...
        for (const mod of modified) {
            if ('before' in mod && 'after' in mod) {
                const change = mod;
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   ✅ Modified (re-export): '${change.after.name}' (${change.before.sourceName} → ${change.after.sourceName})`);
            }
            else {
                const exp = mod;
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   Modified: '${exp.name}'`);
            }
        }
        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========== compareExports END ==========`);
        return {
            added,
            removed,
        ...
        if (initializer) {
            try {
                ...
                if (returnType === 'any' && this.checker && varDecl.compilerNode) {
                    try {
                        const symbol = this.checker.getSymbolAtLocation(varDecl.compilerNode);
                        if (symbol) {
                            const type = this.checker.getTypeOfSymbolAtLocation(symbol, varDecl.compilerNode);
                            const callSignatures = type.getCallSignatures();
    ...
    createInterfaceSymbolInfo(intf) {
        ...
        const properties = [];
        // FIRST: Check if the interface itself is callable (has a call signature)
        // Example: interface Transformer { (data: any, headers?: AxiosRequestHeaders): any; }
        let interfaceCallSignature = undefined;
        try {
            const tsApi = this.getTsApi();
            const compilerNode = intf.compilerNode;
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========== INTERFACE CALL SIGNATURE CAPTURE ==========`);
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Interface: '${name}'`);
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] File: ${intf.getSourceFile().getFilePath()}`);
            // Log TypeScript versions for debugging
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] tsApi version: ${tsApi.version}`);
            try {
                const tsMorph = require("ts-morph");
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ts-morph ts version: ${tsMorph.ts.version}`);
            }
            catch (error) {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Failed to get ts-morph ts version: ${error}`);
            }
            // Get interface members directly from AST
            const members = compilerNode.members || [];
            const sourceFile = compilerNode.getSourceFile();
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Interface members count: ${members.length}`);
            // Count member kinds and log raw member text
            const memberKinds = new Map();
            let callSignatureCount = 0;
            for (let i = 0; i < members.length; i++) {
                const member = members[i];
                const kind = tsApi.SyntaxKind[member.kind] || `Unknown(${member.kind})`;
                memberKinds.set(kind, (memberKinds.get(kind) || 0) + 1);
                // Log raw member text - this is the key debugging info
                const memberText = member.getText(sourceFile);
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Member ${i} (${kind}): "${memberText}"`);
                if (tsApi.isCallSignatureDeclaration(member)) {
                    callSignatureCount++;
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]   ✅ This is a CallSignatureDeclaration!`);
                }
            }
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Member kinds: ${Array.from(memberKinds.entries()).map(([k, v]) => `${k}:${v}`).join(', ')}`);
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] CallSignatureDeclaration count: ${callSignatureCount}`);
            if (callSignatureCount > 0) {
                // Find the first call signature declaration
                const callSigDecl = members.find(m => tsApi.isCallSignatureDeclaration(m));
                if (callSigDecl) {
                    // Build signature directly from the CallSignatureDeclaration node
                    interfaceCallSignature = this.buildCallSignatureFromNode(callSigDecl, sourceFile);
                    if (interfaceCallSignature) {
                        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ✅ Interface '${name}' has call signature:`);
                        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    Parameters: ${JSON.stringify(interfaceCallSignature.parameters.map(p => ({
                            name: p.name,
                            type: p.type,
                            optional: p.optional
                        })))}`);
                        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    Return type: ${interfaceCallSignature.returnType}`);
                        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer]    Formatted: ${this.formatMethodSignatureType(interfaceCallSignature)}`);
                    }
                    else {
                        (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ❌ Interface '${name}' has CallSignatureDeclaration but failed to extract signature`);
                    }
                }
                else {
                    (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ❌ Interface '${name}' callSignatureCount > 0 but could not find CallSignatureDeclaration node`);
                }
            }
            else {
                (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ❌ Interface '${name}' does NOT have a call signature (CallSignatureDeclaration count: ${callSignatureCount})`);
            }
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ========== END CALL SIGNATURE CAPTURE ==========`);
        }
        catch (error) {
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] ❌ Failed to extract call signature from interface '${name}': ${error}`);
        }
        ...
        // Build stable signature string that includes call signature
        let stableSignature = `interface ${name}`;
        if (interfaceCallSignature) {
            const callSigString = this.formatMethodSignatureType(interfaceCallSignature);
            stableSignature = `interface ${name} ${callSigString}`;
        }
        return {
            name,
            qualifiedName: name,
            ...
            metadata: {
                ...
                callSignature: interfaceCallSignature, // Store FunctionSignature object for detailed comparison
                callSignatureString: interfaceCallSignature ? this.formatMethodSignatureType(interfaceCallSignature) : undefined // Store string representation
            }
        };
    }
    /**
     * Build FunctionSignature directly from CallSignatureDeclaration node
     * Extracts parameters and return type from the AST node itself
     */
    buildCallSignatureFromNode(callSigDecl, sourceFile) {
        try {
            const tsApi = this.getTsApi();
            const parameters = [];
            // Extract parameters from the call signature declaration
            if (callSigDecl.parameters && callSigDecl.parameters.length > 0) {
                for (const param of callSigDecl.parameters) {
                    let paramName = 'param';
                    if (param.name) {
                        if (tsApi.isIdentifier(param.name)) {
                            paramName = param.name.text;
                        }
                        else if (tsApi.isObjectBindingPattern(param.name) || tsApi.isArrayBindingPattern(param.name)) {
                            paramName = '...'; // Destructuring pattern
                        }
                    }
            ...
            // Extract return type
            const returnType = callSigDecl.type
                ? callSigDecl.type.getText(sourceFile)
                : 'any';
            return {
                parameters,
                returnType
            ...
            (0, debug_logger_js_1.debugLog)(`[TypeScriptAnalyzer] Failed to build call signature from node: ${error}`);
            return undefined;
        }
    }
    createTypeAliasSymbolInfo(typeAlias) {
        ...
        if (typeNode) {
            try {
                ...
                // Handle function types: type Transformer = (data: any, headers?: AxiosRequestHeaders) => any;
                if (tsApi.isFunctionTypeNode(compilerNode) && this.checker) {
                    try {
                        const type = this.checker.getTypeAtLocation(compilerNode);
                        const callSignatures = type.getCallSignatures();
                        if (callSignatures.length > 0) {
                            const tsSig = callSignatures[0];
                            const methodSignature = this.buildMethodSignature(tsSig, compilerNode);
                            // Store as a property with method signature for comparison
                            // Use a special name to indicate this is the function type itself
                            properties.push({
                                name: '__functionType',
                                type: this.formatMethodSignatureType(methodSignature),
                                methodSignature: methodSignature
                        ...
                        console.log(`[TypeScriptAnalyzer] Failed to extract function type signature: ${error}`);
                    }
                }
                // Handle direct object type literal: type X = { a?: string }
                else if (tsApi.isTypeLiteralNode(compilerNode)) {
    ...
    extractPropertiesFromTypeLiteral(typeLiteral, properties, indexSignatures) {
        ...
        for (const member of members) {
            ...
            if (tsApi.isPropertySignature(member)) {
                ...
                if (propInfo) {
                    // Check if this property has a function type (call signatures)
                    if (this.checker) {
                        try {
                            ...
                            if (callSignatures.length > 0) {
                                // This is a method signature - extract with parameters
                                const tsSig = callSignatures[0];
                                const methodSignature = this.buildMethodSignature(tsSig, member);
                                properties.push({
                                    ...propInfo,
                                    type: this.formatMethodSignatureType(methodSignature),
                                    methodSignature: methodSignature
                            ...
                            console.log(`[TypeScriptAnalyzer] Failed to extract method signature from property: ${error}`);
                        }
                    }
            ...
            // Handle method signatures: { method(): void }
                ...
                if (this.checker) {
                    try {
                        const methodType = this.checker.getTypeAtLocation(member);
                        const callSignatures = methodType.getCallSignatures();
                        if (callSignatures.length > 0) {
                            const tsSig = callSignatures[0];
                            const methodSignature = this.buildMethodSignature(tsSig, member);
                            const methodName = tsApi.isIdentifier(member.name) ? member.name.text : 'unknown';
                            properties.push({
                                name: methodName,
                                type: this.formatMethodSignatureType(methodSignature),
                        ...
                        console.log(`[TypeScriptAnalyzer] Failed to extract method signature: ${error}`);
                    }
                }
            }
        }
    ...
    buildShapeFromTypeChecker(symbol, exportName, identity, decl) {
        ...
        try {
            ...
            if (kind === 'variable') {
                const modifiers = tsApi.canHaveModifiers(targetDecl) ? tsApi.getModifiers(targetDecl) : undefined;
                const isConst = !!(modifiers &&
                    modifiers.some((m) => m.kind === tsApi.SyntaxKind.ConstKeyword));
                return {
        ...
        for (const prop of properties) {
            ...
            if (tsApi.isMethodDeclaration(propDecl) || tsApi.isMethodSignature(propDecl)) {
                ...
                member = {
                    ...
                    readonly: !!(propDecl && tsApi.canHaveModifiers(propDecl) && tsApi.getModifiers(propDecl)?.some((m) => m.kind === tsApi.SyntaxKind.ReadonlyKeyword)),
                    visibility,
                    static: isStatic
                };
            }
                            ...
                            if (tsApi.isIndexSignatureDeclaration(member)) {
                                ...
                                if (keyType && valueType) {
                                    indexSignatures.push({
                                        ...
                                        readonly: !!(tsApi.canHaveModifiers(member) && tsApi.getModifiers(member)?.some((m) => m.kind === tsApi.SyntaxKind.ReadonlyKeyword))
                                    });
                                }
                            }
                        }
    ...
    buildEnumApiShape(symbol, decl) {
        ...
        const name = symbol.getName();
        const members = [];
        const modifiers = tsApi.canHaveModifiers(decl) ? tsApi.getModifiers(decl) : undefined;
        const isConst = !!(modifiers && modifiers.some((m) => m.kind === tsApi.SyntaxKind.ConstKeyword));
