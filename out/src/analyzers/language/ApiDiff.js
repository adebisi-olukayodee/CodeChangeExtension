 ...
 */
function computeApiDiff(before, after) {
    const removed = [];
    const added = [];
    const modified = [];
    const renamed = [];
    ...
    for (const [identity, shape] of before.exports) {
        if (!afterExports.has(identity)) {
            ...
            const matching = afterCandidates.find(c => c.shape.kind === shape.kind &&
 ...
 */
function shapesAreSimilar(shape1, shape2) {
...
function functionShapesEqual(f1, f2) {
...
function classShapesEqual(c1, c2) {
    if (c1.members.length !== c2.members.length) {
        return false;
    }
    const members1 = new Map(c1.members.map((m) => [m.name, m]));
    const members2 = new Map(c2.members.map((m) => [m.name, m]));
...
function typeShapesEqual(t1, t2) {
    if (t1.properties.length !== t2.properties.length) {
        return false;
    }
    const props1 = new Map(t1.properties.map((p) => [p.name, p]));
    const props2 = new Map(t2.properties.map((p) => [p.name, p]));
...
function enumShapesEqual(e1, e2) {
    if (e1.members.length !== e2.members.length) {
        return false;
    }
    const members1 = new Map(e1.members.map((m) => [m.name, m]));
    const members2 = new Map(e2.members.map((m) => [m.name, m]));
    for (const [name, member1] of members1) {
        const member2 = members2.get(name);
        if (!member2 || member1.value !== member2.value) {
            return false;
        }
    }
    return true;
}
function variableShapesEqual(v1, v2) {
 ...
 */
function computeShapeChanges(before, after) {
    const changes = [];
...
function computeFunctionChanges(before, after) {
    const changes = [];
...
function computeClassChanges(before, after) {
    const changes = [];
...
function computeTypeChanges(before, after) {
    const changes = [];
    const propsBefore = new Map(before.properties.map((p) => [p.name, p]));
    const propsAfter = new Map(after.properties.map((p) => [p.name, p]));
    ...
    for (const [name, prop] of propsAfter) {
        ...
        if (!beforeProp) {
            ...
            if (prop.type !== beforeProp.type) {
                changes.push(`Property ${name} type changed from ${beforeProp.type} to ${prop.type}`);
            }
            if (prop.optional !== beforeProp.optional) {
...
function computeEnumChanges(before, after) {
    const changes = [];
    const membersBefore = new Map(before.members.map((m) => [m.name, m]));
    const membersAfter = new Map(after.members.map((m) => [m.name, m]));
    // Find removed members
    ...
    for (const [name, member] of membersAfter) {
        ...
        else if (member.value !== beforeMember.value) {
            changes.push(`Enum member ${name} value changed`);
        }
    }
    return changes;
}
function computeVariableChanges(before, after) {
    const changes = [];
...
//# sourceMappingURL=ApiDiff.js.map