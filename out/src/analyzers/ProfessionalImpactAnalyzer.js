...
class ProfessionalImpactAnalyzer {
    ...
    initializeProject() {
        ...
        this.project = new ts_morph_1.Project({
            useInMemoryFileSystem: false, // Use real file system instead of in-memory
            compilerOptions: {
    ...
    isFunctionUsedInFile(content, functionName, sourceLang, targetLang) {
        ...
        if (targetLang === 'python') {
            ...
            const patterns = [
                new RegExp(`^import\\s+${escapedName}\\b`, 'm'), // import functionName (start of line)
                new RegExp(`^from\\s+.*import\\s+.*${escapedName}\\b`, 'm'), // from ... import ... functionName
            ...
            // Java patterns
            const patterns = [
                new RegExp(`^import\\s+.*\\b${escapedName}\\b`, 'm'), // import statement
                new RegExp(`new\\s+${escapedName}\\s*\\(`, 'g'), // new ClassName(
    ...
    isClassUsedInFile(content, className, sourceLang, targetLang) {
        ...
        if (targetLang === 'python') {
            ...
            const patterns = [
                new RegExp(`^import\\s+${escapedName}\\b`, 'm'), // import ClassName (start of line)
                new RegExp(`^from\\s+.*import\\s+.*${escapedName}\\b`, 'm'), // from ... import ... ClassName
                new RegExp(`\\b${escapedName}\\s*\\(`, 'g'), // ClassName(
            ...
            const patterns = [
                new RegExp(`^import\\s+.*\\b${escapedName}\\b`, 'm'), // import statement
                new RegExp(`new\\s+${escapedName}\\s*\\(`, 'g'), // new ClassName(
                new RegExp(`extends\\s+${escapedName}\\b`, 'g'), // extends ClassName
                ...
                if (funcMatch || methodMatch) {
                    ...
                    if (isInChangedLines) {
                        ...
                        const changedFunction = {
                            ...
                            isExported: indent === 0, // Top-level functions are "exported"
            ...
            for (const cls of allClasses) {
                ...
                if (isClassChanged) {
                    ...
                    const changedClass = {
                        ...
                        properties: [], // Python properties would need separate parsing
