...
export async function analyzeImpactWithDiff(
    ...
    const highConfidenceTests = new Set<string>(testFilesFromDependencyAnalyzer);
 ...
 */
async function testFileImportsSourceFile(
    testFilePath: string,
...
function fileImportsTarget(content: string, targetPath: string, projectRoot: string): boolean {
    ...
    if (packageNameEsc) {
        patterns.push(
            new RegExp(`from\\s+['"]${packageNameEsc}['"]`, 'i'),
            new RegExp(`import\\s+.*\\s+from\\s+['"]${packageNameEsc}['"]`, 'i'),
            new RegExp(`require\\s*\\(\\s*['"]${packageNameEsc}['"]`, 'i')
        );
    }
 ...
 */
function getPackageNameFromFile(filePath: string, projectRoot: string): string | null {
    try {
 ...
 */
async function findTestsThatImportFile(
    sourceFilePath: string,
    projectRoot: string
): Promise<string[]> {
 ...
 */
async function findAllTestFilesInRepo(projectRoot: string): Promise<string[]> {
    const testFiles: string[] = [];
    const testPatterns = [
        /\.test\.(js|jsx|ts|tsx)$/i,
        /\.spec\.(js|jsx|ts|tsx)$/i
