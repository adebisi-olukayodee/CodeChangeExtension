...
export class SimpleImpactViewProvider implements vscode.TreeDataProvider<ImpactViewItem> {
    ...
    private async getFileItems(fileElement: ImpactViewItem): Promise<ImpactViewItem[]> {
        ...
        if (['.js', '.jsx'].includes(fileExt)) {
            const jsWarningItem = new ImpactViewItem(
                '⚠️ JavaScript File - Weaker Analysis Guarantees',
                'js-warning',
                vscode.TreeItemCollapsibleState.None
            );
        ...
        // Impacted Classes
        if (result.changedClasses && result.changedClasses.length > 0) {
            const impactedClassesItem = new ImpactViewItem(
                `Impacted Classes (${result.changedClasses.length})`,
                'impacted-classes',
                vscode.TreeItemCollapsibleState.Collapsed
            );
    ...
    private extractBreakingIssues(result: any): Array<{ severity: string; message: string; line: number; category: string; file?: string; recommendedFixes?: string[] }> {
        ...
        if (uniqueDownstream.length > 0) {
            ...
            const downstreamWithLines = (result as any).downstreamComponentsWithLines || [];
            ...
            for (const component of uniqueDownstream) {
                ...
                const componentWithLines = downstreamWithLines.find((c: any) => {
    ...
    private async getDetailItems(detailElement: ImpactViewItem): Promise<ImpactViewItem[]> {
        ...
        if (detailElement.type === 'breaking-issue') {
            ...
            if (downstreamComponents.length === 0) {
                const noDownstreamItem = new ImpactViewItem(
                    'No downstream impact found in this workspace',
                    'no-downstream',
                    vscode.TreeItemCollapsibleState.None
                );
            ...
            for (const component of downstreamComponents) {
                ...
                const componentWithLines = (safeResult as any).downstreamComponentsWithLines?.find((c: any) => {
                ...
                if (!finalExists) {
                    ...
                    const errorItem = new ImpactViewItem(
                        path.basename(finalPath) + ' (FILE NOT FOUND)',
                        'component',
                        vscode.TreeItemCollapsibleState.None
                    );
                ...
                const componentItem = new ImpactViewItem(
                    path.basename(finalPath) + (firstLine ? ` (line ${firstLine})` : ''),
                    'component',
                    vscode.TreeItemCollapsibleState.None
                );
     ...
     */
    private findImportLine(downstreamFile: string, changedFile: string): number {
        try {
            ...
            if (packageName) {
                ...
                importPatterns.push(
                    new RegExp(`from\\s+['"]${escapedPackageName}['"]`),
                    new RegExp(`import\\s*\\(\\s*['"]${escapedPackageName}['"]`),
                    new RegExp(`require\\s*\\(\\s*['"]${escapedPackageName}['"]`)
                );
            }
