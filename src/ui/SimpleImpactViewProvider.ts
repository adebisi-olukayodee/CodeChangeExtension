...
export class SimpleImpactViewProvider implements vscode.TreeDataProvider<ImpactViewItem> {
    ...
    private extractBreakingIssues(result: any): Array<{ severity: string; message: string; line: number; category: string; file?: string; recommendedFixes?: string[] }> {
        ...
        debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: snapshotDiff exists: ${!!snapshotDiff}`);
        if (snapshotDiff && snapshotDiff.exportChanges) {
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: exportChanges.removed length: ${snapshotDiff.exportChanges.removed?.length || 0}`);
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: exportChanges.modified length: ${snapshotDiff.exportChanges.modified?.length || 0}`);
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: exportChanges.added length: ${snapshotDiff.exportChanges.added?.length || 0}`);
            
            // Check for removed exports
            if (snapshotDiff.exportChanges.removed && snapshotDiff.exportChanges.removed.length > 0) {
                debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Processing ${snapshotDiff.exportChanges.removed.length} removed exports`);
                for (let i = 0; i < snapshotDiff.exportChanges.removed.length; i++) {
                    const removedExport = snapshotDiff.exportChanges.removed[i];
                    debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Removed export ${i}: type=${typeof removedExport}, isObject=${typeof removedExport === 'object'}, keys=${typeof removedExport === 'object' && removedExport !== null ? Object.keys(removedExport).join(',') : 'N/A'}`);
                    if (typeof removedExport === 'object' && removedExport !== null) {
                        debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Removed export ${i} object: ${JSON.stringify(removedExport, null, 2)}`);
                    }
                    const exportName = typeof removedExport === 'object' && removedExport !== null && 'name' in removedExport
                        ? (removedExport as any).name 
                        : typeof removedExport === 'string'
                        ? removedExport
                        : 'unknown';
                    const exportLine = typeof removedExport === 'object' && removedExport !== null && 'line' in removedExport
                        ? (removedExport as any).line 
                        : 0;
                    // Check if there are specific breaking changes for this exported symbol
                    const specificChanges = snapshotDiff?.changedSymbols?.filter((s: any) => 
                        s.isBreaking && 
                    ...
                    if (specificChanges.length > 0) {
                        const specificMessages = specificChanges.map((c: any) => c.metadata?.message || `${c.changeType} detected`).join('; ');
                        message += ` (also: ${specificMessages})`;
                    }
                    
                    debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: ✅ Adding breaking issue for removed export: '${exportName}' at line ${exportLine}, specific changes: ${specificChanges.length}`);
                    breakingIssues.push({
                        ...
                        recommendedFixes: [
                            ...
                            specificChanges.length > 0 ? specificChanges.map((c: any) => c.metadata?.message).filter(Boolean).join('. ') : '',
                ...
                debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: No removed exports found (removed array is ${snapshotDiff.exportChanges.removed ? 'empty' : 'undefined/null'})`);
            }
            ...
            if (snapshotDiff.exportChanges.modified && snapshotDiff.exportChanges.modified.length > 0) {
                debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Processing ${snapshotDiff.exportChanges.modified.length} modified exports`);
                for (const modifiedExport of snapshotDiff.exportChanges.modified) {
                    ...
                    const exportLine = typeof modifiedExport === 'object' && modifiedExport.line 
                        ...
                        : 0;
                    debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: ✅ Adding breaking issue for modified export: '${exportName}' at line ${exportLine}`);
                    breakingIssues.push({
                        severity: '🚨 Breaking Change',
                ...
                debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: No modified exports found (modified array is ${snapshotDiff.exportChanges.modified ? 'empty' : 'undefined/null'})`);
            }
        } else {
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: snapshotDiff or exportChanges is missing`);
            if (snapshotDiff) {
                debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: snapshotDiff exists but exportChanges is ${snapshotDiff.exportChanges ? 'present' : 'missing'}`);
            }
        }
        ...
        if (uniqueDownstream.length > 0) {
            ...
            const downstreamFilesLineNumbers = (result as any).downstreamFilesLineNumbers as Record<string, number> | undefined;
            
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: downstreamFilesLineNumbers exists: ${!!downstreamFilesLineNumbers}`);
            if (downstreamFilesLineNumbers) {
                const keys = Object.keys(downstreamFilesLineNumbers);
                debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Line numbers map has ${keys.length} entries`);
                debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Sample keys: ${keys.slice(0, 3).join(', ')}`);
                // Debug: Show actual values in the map
                keys.slice(0, 3).forEach(key => {
                    debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Map entry: '${key}' = ${downstreamFilesLineNumbers[key]}`);
                });
            }
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: uniqueDownstream count: ${uniqueDownstream.length}, sample: ${uniqueDownstream.slice(0, 3).join(', ')}`);
            ...
            for (const component of uniqueDownstream) {
                ...
                const analysisRoot = (result as any).analysisRootAbs;
                ...
                if (!fsModule.existsSync(absoluteComponentPath)) {
                    debugLog(`[SimpleImpactViewProvider] ⚠️ File not found: ${absoluteComponentPath} (root mismatch - analysisRoot: ${analysisRoot || 'none'})`);
                ...
                let importLine: number | undefined = undefined;
                if (downstreamFilesLineNumbers) {
                    ...
                    debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Exact match lookup for '${component}' (absolute: '${absoluteComponentPath}'): ${importLine !== undefined ? importLine : 'not found'}`);
                    ...
                    if (importLine === undefined) {
                        ...
                        debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Trying normalized match for '${normalizedComponent}' and '${normalizedAbsolute}'`);
                        for (const [key, lineNum] of Object.entries(downstreamFilesLineNumbers)) {
                            ...
                            if (normalizedKey === normalizedComponent || normalizedKey === normalizedAbsolute) {
                                ...
                                if (lineNum !== undefined && lineNum !== null) {
                                    importLine = lineNum;
                                    debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Found line number via path normalization: '${component}' -> '${key}' = ${lineNum}`);
                                    break;
                                }
                            }
                        }
                    ...
                    if (importLine !== undefined) {
                        debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: ✅ Found line number for '${component}': ${importLine}`);
                    } else {
                        debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: ❌ Line number not found for '${component}' in map`);
                    }
                }
                
                // Fallback to finding import line manually if not available
                if (importLine === undefined) {
                    debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Using findImportLine fallback for '${component}'`);
                    ...
                    if (fallbackLine !== undefined && fallbackLine !== null) {
                        ...
                        debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: findImportLine returned: ${importLine}`);
                    } else {
                        debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: findImportLine returned undefined/null`);
                    }
                }
        ...
        if (uniqueAffectedTests.length > 0) {
            ...
            for (const test of uniqueAffectedTests) {
                ...
                if (!fsModule.existsSync(absoluteTestPath)) {
                    debugLog(`[SimpleImpactViewProvider] ⚠️ Test file not found: ${absoluteTestPath} (root mismatch - analysisRoot: ${analysisRoot || 'none'})`);
                }
        ...
        if (snapshotDiff && snapshotDiff.changedSymbols) {
            const breakingSymbolChanges = snapshotDiff.changedSymbols.filter((s: any) => s.isBreaking);
            
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: snapshotDiff.changedSymbols count: ${snapshotDiff.changedSymbols.length}`);
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: breaking changes count: ${breakingSymbolChanges.length}`);
            breakingSymbolChanges.forEach((change: any, idx: number) => {
                debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Breaking change ${idx}: symbol=${change.symbol?.name || 'unknown'}, kind=${change.symbol?.kind || 'unknown'}, message=${change.metadata?.message || 'no message'}, changeType=${change.changeType || 'unknown'}`);
            });
        ...
        if (isHighRiskBreakingChange) {
            ...
            const hasDetailedInfo = snapshotDiff?.changedSymbols?.some((s: any) => s.isBreaking && 
            ...
            const hasDetailedClassInfo = snapshotDiff?.changedSymbols?.some((s: any) => s.isBreaking && s.symbol.kind === 'class');
            ...
            if (uniqueChangedFunctions.length === 0 && uniqueChangedClasses.length === 0 && 
                (!snapshotDiff?.changedSymbols || snapshotDiff.changedSymbols.filter((s: any) => s.isBreaking).length === 0)) {
                breakingIssues.push({
                    severity: '🚨 Breaking Change',
        ...
        debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Final result - ${dedupedIssues.length} breaking issues (${breakingIssues.length} before deduplication)`);
        if (dedupedIssues.length > 0) {
            ...
            debugLog(`[SimpleImpactViewProvider] extractBreakingIssues: Issues by category: ${Array.from(byCategory.entries()).map(([cat, count]) => `${cat}:${count}`).join(', ')}`);
        }
            ...
            for (const issue of issues) {
                ...
                if (issue.line >= 0 && (issue.file || filePath)) {
                    ...
                    if (targetPath && !pathModule.isAbsolute(targetPath)) {
                        const analysisRoot = (inferredResult as any)?.analysisRootAbs;
                        if (analysisRoot) {
                    ...
                    issueItem.command = {
                        ...
                        debugLog(`[SimpleImpactViewProvider] ⚠️ File not found: ${targetPath} (root mismatch)`);
                    }
                } else if (issue.file) {
                    ...
                    issueItem.command = {
                        ...
                        debugLog(`[SimpleImpactViewProvider] ⚠️ File not found: ${fileToOpen} (root mismatch - analysisRoot: ${(inferredResult as any)?.analysisRootAbs || 'none'})`);
            ...
            if (subMetric.issues && Array.isArray(subMetric.issues)) {
                for (const issue of subMetric.issues) {
                    if (typeof issue === 'object' && issue.message && issue.line !== undefined) {
                        ...
                        const issueItem = new ImpactViewItem(
                            `Line ${displayLine}: ${issue.message}`,
                            'issue',
                            vscode.TreeItemCollapsibleState.None
                        );
     ...
     */
    private findImportLine(downstreamFile: string, changedFile: string): number | undefined {
        try {
