...
import { SymbolSnapshot, SymbolInfo, SnapshotDiff, SymbolChange, ExportInfo, PackageSnapshot } from './SymbolSnapshot';
...
export class JavaScriptAnalyzer implements ILanguageAnalyzer {
    ...
    async findReferences(symbolName: string, filePath: string, projectRoot: string): Promise<string[]> {
    ...
    async fileUsesSymbol(filePath: string, symbolName: string, projectRoot: string): Promise<boolean> {
    ...
    async findChangedElements(beforeContent: string, afterContent: string, filePath: string): Promise<{
     ...
     */
    private detectModuleSystemFromCode(content: string): 'cjs' | 'esm' | 'mixed' | 'unknown' {
     ...
     */
    private readPackageSnapshot(packageJsonPath?: string): PackageSnapshot | undefined {
        if (!packageJsonPath) return undefined;
        if (!fs.existsSync(packageJsonPath)) return { type: 'missing', exports: undefined };
    ...
    async buildSnapshot(filePath: string, content: string, packageJsonPath?: string): Promise<SymbolSnapshot> {
        ...
        const imports: any[] = [];
        const cjsExports: ExportInfo[] = [];
        const moduleSystem: 'esm' | 'cjs' | 'mixed' = 'esm';
     ...
     */
    private isExportsMapEmpty(exportsVal: unknown): boolean {
        if (exportsVal == null) return true;
        if (typeof exportsVal !== 'object') return false; // string like "./dist/index.js" is not "empty"
        if (Array.isArray(exportsVal)) return exportsVal.length === 0;
        return Object.keys(exportsVal as Record<string, unknown>).length === 0;
    }

    /**
     * Diff package.json metadata (exports map, type)
     */
    private diffPackageJson(beforeSnapshot: SymbolSnapshot, afterSnapshot: SymbolSnapshot): Array<{
        ruleId: string;
     ...
     */
    private extractExports(sourceFile: ts.SourceFile, exports: ExportInfo[]): void {
     ...
     */
    private extractCjsExports(sourceFile: ts.SourceFile, cjsExports: ExportInfo[]): void {
        const getLineNumber = (node: ts.Node): number => {
            return sourceFile.getLineAndCharacterOfPosition(node.getStart()).line + 1;
        };
    ...
    private detectModuleSystem(
        ...
        esmExports: ExportInfo[],
        cjsExports: ExportInfo[]
    ): 'esm' | 'cjs' | 'mixed' {
        if (esmExports.length > 0 && cjsExports.length > 0) return 'mixed';
        if (esmExports.length > 0) return 'esm';
        if (cjsExports.length > 0) return 'cjs';
        return 'esm'; // Default
    }
     ...
     * Detect module system changes (CJS <-> ESM)
     */
    private detectModuleSystemChange(
        beforeEsExports: ExportInfo[],
        beforeCjsExports: ExportInfo[],
        afterEsExports: ExportInfo[],
        afterCjsExports: ExportInfo[],
     ...
     */
    private extractExportedFunctions(sourceFile: ts.SourceFile, functions: SymbolInfo[]): void {
        const getLineNumber = (node: ts.Node): number => {
            return sourceFile.getLineAndCharacterOfPosition(node.getStart()).line + 1;
        };

        const getColumnNumber = (node: ts.Node): number => {
     ...
     */
    private extractExportedClasses(sourceFile: ts.SourceFile, classes: SymbolInfo[]): void {
        const getLineNumber = (node: ts.Node): number => {
            return sourceFile.getLineAndCharacterOfPosition(node.getStart()).line + 1;
        };
    ...
    private compareExports(
        ...
        const exportKey = (e: ExportInfo): string => {
        ...
        const potentialTypeChanges: Array<{ before: ExportInfo; after: ExportInfo; confidence: number }> = [];
                ...
                if (beforeExport.type === 'named' && beforeExport.name !== '*') {
                    ...
                    for (const [afterKey, afterExport] of afterMap) {
                        if (afterExport.type === 'default' && !processedAdditions.has(afterKey)) {
                            ...
                            let confidence = 0;
                            if (beforeExport.line === afterExport.line) confidence += 2; // Strong indicator
                            if (beforeExport.kind === afterExport.kind && beforeExport.kind !== 'unknown') confidence += 1;
                            if (!beforeExport.sourceModule && !afterExport.sourceModule) confidence += 1;
                    ...
                    for (const [afterKey, afterExport] of afterMap) {
                        if (afterExport.type === 'named' && afterExport.name !== '*' && !processedAdditions.has(afterKey)) {
                            // Confidence checks for pairing:
                            let confidence = 0;
                            if (beforeExport.line === afterExport.line) confidence += 2;
                            if (beforeExport.kind === afterExport.kind && beforeExport.kind !== 'unknown') confidence += 1;
                            if (!beforeExport.sourceModule && !afterExport.sourceModule) confidence += 1;
            ...
            if (!afterMap.has(key) && !processedRemovals.has(key)) {
                ...
                changedSymbols.push({
                    symbol: {
                        ...
                        isExported: true,
                        kind: beforeExport.kind as any || 'variable'
                    },
                    changeType: 'removed',
                ...
                if (beforeExport.type !== afterExport.type ||
                    ...
                    if (beforeExport.type !== afterExport.type) {
                        ...
                        const beforeType = beforeExport.type as string;
                        const afterType = afterExport.type as string;
                    ...
                    changedSymbols.push({
                        symbol: {
                            ...
                            isExported: true,
                            kind: afterExport.kind as any || 'variable'
                        },
                        changeType: 'modified',
                        before: {
                            name: beforeExport.name,
    ...
    private compareFunctions(
        ...
        removedExports: Set<string> = new Set()
    ): void {
        ...
        for (const [name, beforeFunc] of beforeMap) {
            if (!afterMap.has(name)) {
                ...
                const beforeParamCount = beforeFunc.metadata?.paramCount as number | undefined;
                const afterParamCount = afterFunc.metadata?.paramCount as number | undefined;
                const beforeHasRest = beforeFunc.metadata?.hasRestParam as boolean | undefined;
                const afterHasRest = afterFunc.metadata?.hasRestParam as boolean | undefined;
