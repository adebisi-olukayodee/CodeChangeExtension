...
import { ApiDiff, ApiShape, FunctionApiShape, ClassApiShape, TypeApiShape, EnumApiShape } from './ApiSnapshotTypes.js';
import { BreakingChangeRule } from '../../types/EnhancedImpactReport.js';
import { Finding } from '../../regression-runner.js';

/**
 * Converts an API diff into breaking change findings with rule IDs.
 */
export function apiDiffToFindings(apiDiff: ApiDiff, entrypointPath: string): Finding[] {
 ...
 */
function getRemovedExportRuleId(shape: ApiShape): string {
    switch (shape.kind) {
        case 'function':
            return BreakingChangeRule.FN_REMOVED;
        case 'class':
        ...
        case 'variable':
        case 'const':
            return BreakingChangeRule.EXPORT_REMOVED;
        default:
            return BreakingChangeRule.EXPORT_REMOVED;
    }
}
 ...
 */
function getModifiedExportFindings(
    before: ApiShape,
    after: ApiShape,
 ...
 */
function getFunctionChangeFindings(
    before: FunctionApiShape,
    after: FunctionApiShape,
    entrypointPath: string
): Finding[] {
    const findings: Finding[] = [];
    ...
    if (before.overloads.length !== after.overloads.length) {
        findings.push({
            ruleId: BreakingChangeRule.FN_OVERLOAD_CHANGED,
            severity: 'breaking',
            symbol: before.name,
            file: entrypointPath,
    ...
    for (let i = 0; i < maxOverloads; i++) {
        ...
        if (beforeOverload.returnType !== afterOverload.returnType) {
            findings.push({
                ruleId: BreakingChangeRule.FN_RETURN_TYPE_CHANGED,
                severity: 'breaking',
                symbol: before.name,
                file: entrypointPath,
        ...
        for (const [name, beforeParam] of beforeParams) {
            if (!afterParams.has(name)) {
                findings.push({
                    ruleId: BreakingChangeRule.FN_PARAM_REMOVED,
                    severity: 'breaking',
                    symbol: before.name,
                    file: entrypointPath,
                ...
                if (beforeParam.type !== afterParam.type) {
                    findings.push({
                        ruleId: BreakingChangeRule.FN_PARAM_TYPE_CHANGED,
                        severity: 'breaking',
                        symbol: before.name,
                        file: entrypointPath,
                ...
                if (beforeParam.optional && !afterParam.optional) {
                    findings.push({
                        ruleId: BreakingChangeRule.FN_PARAM_REQUIRED,
                        severity: 'breaking',
                        symbol: before.name,
                        file: entrypointPath,
 ...
 */
function getClassChangeFindings(
    before: ClassApiShape,
    after: ClassApiShape,
    entrypointPath: string
): Finding[] {
    const findings: Finding[] = [];
    ...
    for (const [name, beforeMember] of beforeMembers) {
        if (!afterMembers.has(name)) {
            const ruleId = beforeMember.kind === 'method' 
                ? BreakingChangeRule.CLS_METHOD_REMOVED
                : BreakingChangeRule.CLS_PROPERTY_REMOVED;
            ...
            if (beforeMember.kind === 'method' && afterMember.kind === 'method') {
                if (beforeMember.signature && afterMember.signature) {
                    ...
                    if (beforeSig !== afterSig) {
                        findings.push({
                            ruleId: BreakingChangeRule.CLS_METHOD_SIGNATURE_CHANGED,
                            severity: 'breaking',
                            symbol: before.name,
                            file: entrypointPath,
 ...
 */
function getTypeChangeFindings(
    before: TypeApiShape,
    after: TypeApiShape,
    entrypointPath: string
): Finding[] {
    ...
    for (const [name, beforeProp] of beforeProps) {
        if (!afterProps.has(name)) {
            const ruleId = before.kind === 'interface'
                ? BreakingChangeRule.IFACE_PROPERTY_REMOVED
                : BreakingChangeRule.TYPE_PROPERTY_TYPE_CHANGED; // Use as fallback
            ...
            if (beforeProp.optional && !afterProp.optional) {
                const ruleId = before.kind === 'interface'
                    ? BreakingChangeRule.IFACE_PROPERTY_REQUIRED
                    : BreakingChangeRule.TYPE_PROPERTY_REQUIRED;
            ...
            if (beforeProp.type !== afterProp.type) {
                const ruleId = before.kind === 'interface'
                    ? BreakingChangeRule.IFACE_PROPERTY_TYPE_CHANGED
                    : BreakingChangeRule.TYPE_PROPERTY_TYPE_CHANGED;
    ...
    if (before.typeText && after.typeText && before.typeText !== after.typeText) {
        findings.push({
            ruleId: BreakingChangeRule.TYPE_DEFINITION_CHANGED,
            severity: 'breaking',
            symbol: before.name,
            file: entrypointPath,
 ...
 */
function getEnumChangeFindings(
    before: EnumApiShape,
    after: EnumApiShape,
    entrypointPath: string
): Finding[] {
    ...
    for (const [name, beforeMember] of beforeMembers) {
        if (!afterMembers.has(name)) {
            findings.push({
                ruleId: BreakingChangeRule.ENUM_MEMBER_REMOVED,
                severity: 'breaking',
                symbol: before.name,
                file: entrypointPath,
