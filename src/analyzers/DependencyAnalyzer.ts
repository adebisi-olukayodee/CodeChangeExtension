...
interface ExportInfo {
    ...
    to: string;   // Resolved absolute path of re-exported file (if re-export)
    exportName: string; // Name of the export
    isReExport: boolean; // true if this is a re-export (export * from or export {x} from)
    isNamespaceReExport: boolean; // true if export * from (namespace re-export)
}
...
export class DependencyAnalyzer {
    ...
    private readonly MAX_FILES_TO_ANALYZE = 10000; // Maximum files to process in dependency graph
    private readonly MAX_FILE_SIZE_BYTES = 5 * 1024 * 1024; // 5MB max file size
     ...
     */
    private trackExports(filePath: string, exports: ExportInfo[]): void {
     ...
     */
    private createModuleResolutionHost(): ts.ModuleResolutionHost {
        return {
    ...
    async findDownstreamComponentsWithLines(
        ...
        impactedSymbols?: string[],
        projectRoot?: string
    ): Promise<Array<{ filePath: string; lines: number[] }>> {
        ...
        const reExportingFiles = new Set<string>();
        ...
        const result: Array<{ filePath: string; lines: number[] }> = [];
     ...
     */
    private fileUsesSymbols(filePath: string, symbolNames: string[], sourceFilePath: string): boolean {
     ...
     */
    private fileUsesSymbolsWithLines(filePath: string, symbolNames: string[], sourceFilePath: string): { uses: boolean; lines: number[] } {
        try {
            const content = fs.readFileSync(filePath, 'utf8');
     ...
     */
    private findImportLines(content: string, filePath: string, sourceFilePath: string): number[] {
        const lines: number[] = [];
        const fileDir = path.dirname(filePath);
     ...
     */
    private async findDownstreamByDirectScan(
        sourceFilePath: string,
        projectRoot: string,
        impactedSymbols?: string[]
    ): Promise<Array<{ filePath: string; lines: number[] }>> {
        console.log(`[DependencyAnalyzer] Starting direct scan for: ${sourceFilePath}`);
        const downstreamFiles: Array<{ filePath: string; lines: number[] }> = [];
        const normalizedSource = path.resolve(sourceFilePath).replace(/\\/g, '/');
        const normalizedSourceWin = normalizedSource.replace(/\//g, '\\');
